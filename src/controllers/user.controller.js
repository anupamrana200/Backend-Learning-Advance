...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if(!user){
    throw new ApiError(404, "User does not exists! Please enter correct username or email.")
  }

  //4. Check the password
  const isPasswordValid = await user.isPasswordCorrect(password)
  if(!isPasswordValid){
    throw new ApiError(401, "Invalid user credentials! Please recheck the password.")
  }

  //5. generate a access token and refresh token and send to the user
  ...
  .json(
    new ApiResponse(
      ...
      "Congrats! User Logged In Successfully."
    )
  )
  ...
  try {
    ...
    .json(
      new ApiResponse(
        ...
        "Congrats! Access token Refreshed Successfully."
      )
    )
  } catch (error) {
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if(!isPasswordCorrect){
    throw new ApiError(400, "Old Password, that you entered is wrong!")
  }
